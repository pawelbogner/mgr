...
with system dynamics and the RobRex mobile manipulator with platform dynamics and arm kinematics.
\section{Unicycle}
...
\begin{equation}
\begin{aligned}
A_0^1 &= \rot(Z, x_1)\rot(X, \frac{\pi}{2}),\\
A_1^2 &= \rot(Z, x_2)\tr (X, a_2)\rot(X, -\frac{\pi}{2}),\\
A_2^3 &= \rot(Z, x_3)\tr (X, a_3)\rot(X, \frac{\pi}{2}),\\
A_3^4 &= \rot(Z, x_4)\tr (X, a_4)\rot(X, -\frac{\pi}{2}),\\
A_4^5 &= \rot(Z, x_5)\tr (X, a_5).
\end{aligned}
\end{equation}
